...
public class SRLHttpPipelineSBU extends AbstractPipeline {
	...
	public SRLHttpPipelineSBU(CompletePipeline pl,ImageCache imageCache,L l, int sentenceMaxLength, HttpOptions options){
	...
	public String getParseInterfaceHTML(L l) {
		String s=
		  "  <h3>Semantic Role Labeler for Process Sentences. (Powered by <a href=\"https://code.google.com/p/mate-tools\">MATE</a> )</h3>\n" +
		  "  Enter a sentence in <b>"+Language.getLanguage().toLangNameString()+"</b> (e.g. water evaporates into water vapor.) and press Parse.<br/>\n" +
		  ...
		  "      <tr><td valign=\"center\"><b>Return type</b><td><input type=\"radio\" name=\"returnType\" value=\"html\" checked=\"checked\" />&nbsp;&nbsp;HTML<br /><input type=\"radio\" name=\"returnType\" value=\"text\"/>&nbsp;&nbsp;Raw text<br/></td></tr>\n" +
	...
	public StringPair parseRequest(String inputSentence,Map<String, String> vars) throws Exception {
		long parsingTime=System.currentTimeMillis();
                // tokenize with Stanford
                //list.stream().toArray(String[]::new);
		String[] tokens = StanfordTokenizerSingleton.getInstance().tokenize(inputSentence).stream().toArray(String[]::new);
		if(sentenceMaxLength>0 && tokens.length>sentenceMaxLength)
			throw new AbstractPipeline.SentenceTooLongException("Sentence too long",tokens.length);
                
                // convert to CoNLL'09
                String CoNLL06Str = StanfordDepParserSingleton.getInstance().parseCoNLL(inputSentence);
                ...
                // For now, only one sentence per request
                Sentence sen = sentences.get(0);
...
//			ret.append(pred.getSense()+"</a></td>");
	...
	public static void main(String[] args) throws ZipException, IOException, ClassNotFoundException{
		...
		HttpPipeline.setupHttpPipeline(options, new SRLHttpPipelineSBU(completePipeline,HttpPipeline.imageCache,options.l,options.sentenceMaxLength,options));
